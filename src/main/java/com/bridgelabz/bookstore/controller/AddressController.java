...
public class AddressController {
	...
	public ResponseEntity<Response> updateAddress(@Valid @RequestBody AddressDto addressDto,
			@RequestHeader String token, @RequestHeader Long addressId, BindingResult result)
			throws BookStoreException {
		if (result.hasErrors())
			return new ResponseEntity<Response>(
					new Response(result.getAllErrors().get(0).getDefaultMessage(), null, 400), HttpStatus.BAD_REQUEST);
	...
	public ResponseEntity<Response> deleteAddress(String token, @PathVariable("addressId") Long addressId)
			throws BookStoreException {
	...
	public ResponseEntity<Response> getAddressForUser(@RequestHeader("type") String type,
			@RequestHeader("token") String token) throws BookStoreException {
